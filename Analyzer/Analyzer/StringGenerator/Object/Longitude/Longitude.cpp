...
Longitude::Longitude()
{
	...
	std::uniform_int_distribution<size_t> dot_dis(0, 1); // наличие знака
	std::uniform_int_distribution<size_t> number_dis('0', '9'); // случайная цифра
	size_t length = length_dis(rd);
	for (size_t i = 0; i < length; ++i)
	{
		_name += right_chance_dis(rd) < limit ? static_cast<char>(number_dis(rd)) : get_random_literal(); // собираем долготу
	}
	if (dot_dis(rd) == 1) {
	...
	for (size_t i = 0; i < length; ++i)
	{
		_name += right_chance_dis(rd) < limit ? static_cast<char>(number_dis(rd)) : get_random_literal(); // долгота после точки
	}
	if (dot_dis(rd) == 1) {
